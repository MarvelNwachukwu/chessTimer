  ...
  const startCountDown = () => { 
    ...
    let count = { 
      white: { 
        ...
        mn: (parseInt(whiteData.minutes) > 0) ? parseInt(whiteData.minutes * 60) : 0,
        sc: (parseInt(whiteData.seconds) > 0) ? parseInt(whiteData.seconds) : 0,
      },
      black: { 
        hr: (parseInt(blackData.hours) > 0) ? parseInt(blackData.hours * 3600) : 0,
        mn: (parseInt(blackData.minutes) > 0) ? parseInt(blackData.minutes * 60) : 0,
    ...
    if(target.dataset.color === 'white') { 
      if(target.dataset.type === 'hr') { 
        if(target.value.length <= 2) {
          whiteData.hours.length < target.value.length ?           setDataWatch({...dataWatch, white: true}) :
        ...
        if(target.value.length <= 2) { 
          whiteData.minutes.length < target.value.length ?           setDataWatch({...dataWatch, white: true}) :
        ...
        if(target.value.length <= 2) { 
          whiteData.seconds.length < target.value.length ?           setDataWatch({...dataWatch, white: true}) :
      ...
      if(target.dataset.type === 'hr') { 
        if(target.value.length <= 2) { 
          blackData.hours.length < target.value.length ?           setDataWatch({...dataWatch, black: true}) :
        ...
        if(target.value.length <= 2) { 
          blackData.minutes.length < target.value.length ?           setDataWatch({...dataWatch, black: true}) :
        ...
        if(target.value.length <= 2) { 
          blackData.seconds.length < target.value.length ?           setDataWatch({...dataWatch, black: true}) :
      ...
      <section className='content'>
        ...
        <div className='controls'>
          ...
          <div 
            ...
            style={{opacity: `${(dataWatch.white && dataWatch.black) ? '1' : '0.3'}`, cursor: `${(dataWatch.white && dataWatch.black) ? 'pointer' : 'not-allowed'}`}}
            onClick={(dataWatch.white && dataWatch.black) ? startCountDown : () => {}}
          >
            Start Game
          </div>
          <div id='breaker'></div>
          ...
          <div 
          className={`${dataWatch.white || dataWatch.white ? 'reset' : 'resetDull'}`} 
          style={{opacity: `${whiteData.hours ? '1' : whiteData.minutes ? '1' : whiteData.seconds ? '1' : '0.3'}`, cursor: `${whiteData.hours ? 'pointer' : whiteData.minutes ? 'pointer' : whiteData.seconds ? 'pointer' : 'not-allowed'}`}}
          >
            Reset
          </div>
        </div>
